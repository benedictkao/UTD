...
using string = utd::string24;

static const char* getDataAddress(const string& s) {
  return s.c_str();
}

static const char* getCapacityAddress(const string& s) {
  return ((char*) &s + 1);
}

static bool isSmallString(const string& s) {
  return getDataAddress(s) == getCapacityAddress(s);
}

TEST(string24, default_constructor) {
  utd::string24 s;
  EXPECT_EQ(s.size(), 0);
  EXPECT_TRUE(s.empty());
  EXPECT_EQ(s.capacity(), 24);
  EXPECT_TRUE(s == "");
...
class ConstructorTestFixture
    : public ::testing::TestWithParam<ConstructorArgs> {
protected:
...
TEST_P(ConstructorTestFixture, move_constructor) {
  auto          params = GetParam();
  utd::string24 s      = params.data;
  utd::string24 s2((utd::string24&&) s);
  EXPECT_EQ(s2.size(), params.size);
  EXPECT_EQ(s2.empty(), params.size == 0);
  EXPECT_EQ(s2.capacity(), params.capacity);
  EXPECT_TRUE(s2 == params.data);
...
INSTANTIATE_TEST_SUITE_P(
  ...
  ::testing::Values(ConstructorArgs{ "1234", 4, 24 },
...
TEST_P(AssignmentTestFixture, copy_assignment) {
  auto          params = GetParam();
  utd::string24 s      = params.data1;
  utd::string24 s2     = params.data2;
  s2                   = s;
...
INSTANTIATE_TEST_SUITE_P(
  ...
  ReserveTestFixture,
  ::testing::Values(
    ReserveArgs{ "123", 15, "123", 24 },
    ReserveArgs{ "A heap allocated string.",
                 15,
                 "A heap allocated string.",
                 25 },
    ReserveArgs{ "123", 20, "123", 24 },
    ReserveArgs{ "Heap allocated str", 20, "Heap allocated str", 24 },
    ReserveArgs{ "A heap allocated string for heaps.",
                 30,
                 "A heap allocated string for he",
                 31 }));

TEST(string24, sq_bracket_operator) {
  utd::string24 s = "test";
  EXPECT_EQ(s[0], 't');
  EXPECT_EQ(s[2], 's');
  s[0] = 'T';
...
TEST_P(StrConcatTestFixture, str_concatentation) {
  auto          params = GetParam();
  utd::string24 s      = params.init_str;
  utd::string24 s2     = s + params.add_str_1;
...
TEST_P(CharConcatTestFixture, char_concatentation) {
  ...
  utd::string24 s  = params.init_str;
  utd::string24 s2 = s + params.add_char_1;
  EXPECT_TRUE(s == params.init_str);
  EXPECT_TRUE(s2 == params.str_res_1);
